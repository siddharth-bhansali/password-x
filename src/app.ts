const getRandomIndex = async (length: number): Promise<number> => Math.floor(Math.random() * length);

const shuffleCharacters = async (data: string[]): Promise<string[]> => {
...
if (generatedPasswordText) {
    generatedPasswordText.addEventListener("click", (event) => {
        const password = (event.target as HTMLElement).textContent ?? "";
